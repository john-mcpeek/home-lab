...
export ANSIBLE_PUBLIC_KEY
 ...
 ssh "root@${PROXMOX_IP}" "rm -rf vms/cluster-api-image-builder/*"

 scp -r cluster-api-image-builder/ "root@${PROXMOX_IP}":~/vms

 ssh "root@${PROXMOX_IP}" "cd vms && ./cluster-api-image-builder/generate-cloud-init-files.sh 'capi-base' '${MY_PUBLIC_KEY}' '${ANSIBLE_PUBLIC_KEY}'"
 ssh "root@${PROXMOX_IP}" "cd vms && ./cluster-api-image-builder/build-vm.sh ${PROXMOX_IP}"
